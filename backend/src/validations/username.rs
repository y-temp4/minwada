...
lazy_static! {
    ...
    static ref USERNAME_PATTERN: Regex = Regex::new(r"^[a-zA-Z][a-zA-Z0-9_-]*$").unwrap();
}
...
pub fn validate_username(username: &str) -> Result<(), ValidationError> {
    ...
    if !USERNAME_PATTERN.is_match(username) {
        return Err(ValidationError::new("username_invalid_pattern"));
    }

    // 予約語チェック
    let lowercase_username = username.to_lowercase();
    if RESERVED_USERNAMES.contains(&lowercase_username.as_str()) {
...
// validatorライブラリと連携するための検証関数
pub fn username_validator(username: &str) -> Result<(), ValidationError> {
    validate_username(username)
}

// Optionをサポートするバリデータ関数
pub fn username_optional_validator(username_opt: &Option<String>) -> Result<(), ValidationError> {
    match username_opt {
        Some(username) => validate_username(username),
        None => Ok(()),
    }
}
